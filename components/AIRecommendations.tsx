...
const AIRecommendations: React.FC<AIRecommendationsProps> = ({ profile, courses, language }) => {
  ...
  const getRelevanceColor = (score: number) => {
      ...
      if (score >= 70) return 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-300';
  ...
  return (
    <div className="space-y-6">
      ...
      {!result && !loading && !error && (
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-10 text-center shadow-sm border border-gray-100 dark:border-gray-700 flex flex-col items-center justify-center min-h-[400px] transition-colors">
           <div className="bg-primary-50 dark:bg-primary-900/20 p-4 rounded-full mb-4">
             <BrainCircuitIcon className="w-12 h-12 text-primary-400 dark:text-primary-500" />
           </div>
           <h3 className="text-xl font-semibold text-gray-800 dark:text-white mb-2">{t.readyTitle}</h3>
        ...
        <div className="space-y-8 animate-fade-in">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {result.recommendations.map((rec) => (
              <div 
                key={rec.id} 
                className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 hover:shadow-md transition-all duration-300 flex flex-col"
              >
                <div className="p-5 flex-1">
                   ...
                   <h4 className="text-lg font-bold text-gray-800 dark:text-white mb-2 line-clamp-2 h-14">
                     {rec.title}
                   </h4>
                   ...
                   <div className="bg-gray-50 dark:bg-gray-700/50 p-3 rounded-lg border border-gray-100 dark:border-gray-600 mb-4">
                     <p className="text-xs text-gray-500 dark:text-gray-400 italic">
                       {t.reason}: <span className="font-medium text-gray-600 dark:text-gray-300">{rec.reasoning}</span>
                     </p>
                   </div>
                </div>

                <div className="p-4 border-t border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/30 rounded-b-xl flex justify-between items-center">
                   <span className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">
                      {rec.type}
                   </span>
                   {rec.platformOrLink && (
                      <button className="text-primary-600 dark:text-primary-400 hover:text-primary-800 dark:hover:text-primary-300 text-sm font-medium flex items-center gap-1">
